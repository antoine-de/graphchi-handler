...
fn parse_file(wat_file: PathBuf) -> Result<(), Error> {
    ...
    for l in reader.lines() {
        ...
        info!("base links: {:?}", &links);

        if let &Some(ref l) = links {
            let serde_json_links: Vec<Link> = serde_json::from_str(&l).unwrap_or(vec![]);
            info!("serde_json_links parsed  links: {:?}", &serde_json_links);
            let cassandra_link = Site {
                url: base_url.unwrap_or("".into()),
                uuid: base_id.unwrap_or("".into()),
                links: serde_json_links,
            };
            info!("link to send: {:?}", &cassandra_link);

            let query = format!("INSERT INTO graph.sites (uuid, url, links) VALUES ({}, '{}', {})", 
            base_url.unwrap_or("".into()), 
            &cassandra_link.url, 
            serde_json::to_string(&cassandra_link.links).unwrap());
